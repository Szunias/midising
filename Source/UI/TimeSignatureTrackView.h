...
class TimeSignatureTrackView : public juce::Component
{
public:
    TimeSignatureTrackView();
    ~TimeSignatureTrackView() override = default;
    ...
    void setTimeSignatureTrack(TimeSignatureTrack* track) { timeSigTrack = track; repaint(); }
...
private:
    ...
    void showEditDialog(int eventIndex);
    void showAddDialog(int64_t position);
    int findEventAtPixel(int x) const;

    TimeSignatureTrack* timeSigTrack = nullptr;
